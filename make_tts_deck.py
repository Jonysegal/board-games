...
IMPORTANT ABOUT TTS URLs
- TTS requires FaceURL/BackURL to be publicly accessible URLs.
- This script can optionally fill those URLs if you provide --face-url-base and --back-url.
- Otherwise, it will write placeholders and also write a manifest so you can upload and then replace.

Usage:
  python make_tts_deck.py cards.tsv --out-dir build_tts

Recommended:
  python make_tts_deck.py cards.tsv --out-dir build_tts --per-section \
      --face-url-base "https://raw.githubusercontent.com/<you>/<repo>/main/build_tts/sprites" \
      --back-url "https://raw.githubusercontent.com/<you>/<repo>/main/build_tts/back.png"

Dependencies:
  pip install pillow requests cairosvg

Notes:
- Wikimedia "wiki/File:..." links are normalized to "Special:FilePath/<name>?width=512"
- SVG icons are converted to PNG if cairosvg is available.
"""
...
def parse_card_file_tsvish(path: Path) -> List[CardDef]:
    ...
    # Keep non-empty lines; TSV rows with blanks in columns are still lines.
...
def normalize_wikimedia_url(url: str, width: int = 512) -> str:
    """
    Convert Wikimedia Commons wiki/File: page URL to a direct Special:FilePath URL.
    Adds ?width=<width> to help rasterize SVG on Commons.
    """
    u = url.strip()
...
def pil_from_bytes(data: bytes) -> Image.Image:
    ...
    # Flatten transparency onto white for consistent card rendering
...
def load_icon_image(
    ...
    if is_svg:
        ...
        if png is None:
            # Can't render SVG without cairosvg
            return None
        data = png

    # Cache the final bytes we will actually render
    try:
...
def render_card_png(
    ...
    # Layout constants (MTG-ish)
    pad = int(0.04 * W)  # ~30px on 744
    name_h = int(0.10 * H)  # title bar area
    art_h = int(0.46 * H)
    type_h = int(0.06 * H)
    gap = int(0.012 * H)
    ...
    name_text = card.name
    draw.text((pad + 14, pad + 10), name_text, font=name_font, fill=(0, 0, 0))
    ...
    if icon is not None:
        # Fit icon into art box with padding
...
def choose_grid_for_sheet(
    ...
    """
    Choose (cols, rows) so that:
      cols * card_w <= max_sheet_px
      rows * card_h <= max_sheet_px
    and uses preferred_cols when possible.
    """
    ...
    # Choose a reasonably tall grid (rows computed later from number of cards)
...
def pack_into_sheets(
    ...
    """
    Returns list of tuples: (sheet_path, num_width, num_height, count_on_sheet)
    """
...
def tts_deck_object(
    ...
    """
    Create a single TTS Deck object. Supports multiple sheets via CustomDeck keys.
    CardID convention:
      CardID = (deckIndex * 100) + slotIndex
    where:
      deckIndex starts at 1 for the first sheet in this deck object,
      slotIndex starts at 0..(count_on_sheet-1)
    """
    ...
    # Flatten card list to match packing order:
    # The packer consumes the PNG paths in order; we mirror that order here.
    card_idx = 0
    deck_index = 1
    ...
    for (sheet_path, num_w, num_h, count_on_sheet), face_url in zip(sheet_specs, face_urls):
        ...
        for slot in range(count_on_sheet):
            ...
            # Keep Description concise; TTS shows full text in hover and in notebook
            desc = card.text
            ...
            contained.append(
                {
                    ...
                    "Description": desc,
                    "CardID": card_id,
                    "CustomDeck": {},  # TTS ignores this here; uses parent CustomDeck
                }
            )
            card_idx += 1
...
def build_face_urls(sheet_specs: List[Tuple[Path, int, int, int]], face_url_base: str) -> List[str]:
    """
    If face_url_base is provided, generate URLs like:
      <base>/deck_01.png
...
def main():
    ...
    ap.add_argument("--face-url-base", type=str, default="", help="Base URL where deck_XX.png sheets will be hosted")
    ap.add_argument("--back-url", type=str, default="", help="Public URL for back.png (optional). If omitted, placeholder is used.")
    ...
    if not args.single_deck and not args.per_section:
        # Default to per-section (more manageable in TTS)
    ...
    back_url = args.back_url.strip()
    if not back_url:
        back_url = "REPLACE_WITH_PUBLIC_URL/back.png"
    ...
    if args.single_deck:
        ...
        # Stable order: sort by section name
        for sec in sorted(by_section.keys()):
            groups.append((sec, by_section[sec]))

    # Render PNG faces and pack to sheets per group; build TTS objects
    tts_objects: List[dict] = []
    manifest_rows: List[List[str]] = []

    base_x = 0.0
    base_z = 0.0
    step_x = 3.5  # spacing between decks on table
    step_z = 0.0
    ...
    for gi, (group_name, group_cards) in enumerate(groups):
        ...
        # Render faces
        group_face_dir = faces_dir / re.sub(r"[^a-zA-Z0-9_-]+", "_", group_name).strip("_")
        ...
        group_sprite_dir = sprites_dir / re.sub(r"[^a-zA-Z0-9_-]+", "_", group_name).strip("_")
    ...
    # Emit TTS JSON (as a Saved Object snippet)
    # You can paste this into TTS's "Saved Objects" JSON or use it in a mod save file.
    ...
    out_manifest.write_text(
        "deck_name\tsheet_file\tsheet_path\tnum_width\tnum_height\tcards_on_sheet\tface_url\n" +
        "\n".join("\t".join(row) for row in manifest_rows),
        encoding="utf-8",
    )

    # Emit expanded card list (debugging + iteration)
